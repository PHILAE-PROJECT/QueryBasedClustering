...
class SequencesStringComparisonClustering(object):
    ...
    """
    @staticmethod
    def are_patterns_similar(pattern1: list, pattern2: list,
                             consider_order_of_sequence: bool = True) -> bool:
    ...
    @staticmethod
    def _helper_validate_pattern_inputs(
            subsequence_size: int,
            consider_order_of_sequence: bool,
            consider_immediate_occurrence: bool,
            consider_duplicate_values: bool,
            immediate_occurrence_max: int
    ) -> bool:
        """
    ...
    @staticmethod
    def create_pattern(
            from_sequence: list, subsequence_size: int,
            consider_order_of_sequence: bool = True,
            consider_immediate_occurrence: bool = True,
            consider_duplicate_values: bool = True,
        ...
        _ = SequencesStringComparisonClustering._helper_validate_pattern_inputs(
                ...
                consider_duplicate_values=consider_duplicate_values,
                immediate_occurrence_max=immediate_occurrence_max
        )
    ...
    @staticmethod
    def create_clusters(
            sequences: list, subsequence_size: int = 1,
            consider_order_of_sequence: bool = True,
            consider_immediate_occurrence: bool = True,
            consider_duplicate_values: bool = True,
            immediate_occurrence_max: int = 2
    ...
    @staticmethod
    def example_create_pattern():
        sequence1 = [["l", 0], ["r", 0], ["l", -1], ["l", -2],
                     ["r", -3], ["r", -1], ["r", -2], ["n", 0]]

        dict_result = dict()
        sequence = [["a"], ["a"], ["a"], ["a"], ["b"], ["c"],
                    ["d"], ["d"], ["d"], ["d"], ["a"], ["a"], ["a"], ["a"]]
        # sequence = ["a", "a", "a", "a", "b", "c",
        #             "d", "d", "d", "d", "a", "a", "a", "a"]
        for cos in [True, False]:
            for cio in [True, False]:
                for cdv in [True, False]:
                    parameter = f"cos_{cos}_cio_{cio}_cdv_{cdv}"
                    res = SequencesStringComparisonClustering.create_pattern(
                        from_sequence=sequence, subsequence_size=1,
                        consider_duplicate_values=cdv, consider_order_of_sequence=cos,
                        consider_immediate_occurrence=cio)
                    dict_result[parameter] = res
        return dict_result
    # end example_create_pattern
